...
class StreetGraph:
    ...
    def graphGeometry(self, id, file=None):
        ...
        fig.suptitle(
            f"Circular Pipe Functions vs Central Angle θ\n",
            fontsize=16,
            fontweight="bold",
        )
        ...
        axes[1, 1].axhline(
            y=self.G.es["hydraulicRadiusFull"][id],
            color="r",
            linestyle="--",
            alpha=0.5,
            label="Full",
    ...
    def __init__(self, file=None):
        ...
        if file == None:
            ...
            for e in self.G.es:
                ...
                self.G.es[e.index]["slope"] = (
                    self.G.vs[e.source]["z"] - self.G.vs[e.target]["z"]
                )
            ...
            self.G.es["hydraulicRadiusFull"] = np.multiply(0.25, self.G.es["diam"])
            self.G.es["sectionFactorFull"] = self.G.es["Amax"] * np.power(
                self.G.es["hydraulicRadiusFull"], 2 / 3
            )
            ...
            for e in self.G.es:
                ...
                if slope < 0.00003048:
                    print(
                        f"WARNING: slope for edge ({e.source}, {e.target}) is too small."
                    )
                    print(
                        f"{e.source}: ({self.G.vs[e.source]['x']}, {self.G.vs[e.source]['y']}, {self.G.vs[e.source]['z']})"
                    )
